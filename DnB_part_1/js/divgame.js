...
function buildTable (rows, cols) {	
	//If a table already exists, remove it before creating a new one, other	
	removeTable(), 
	
    //Keeping the table state as an array
...
function horizontalMove(pos, user){
    ...
    //If player just maded a square or not, if it does makes another move
	if (user == "p") {
		var a = document.getElementById(row+','+col);
		a.className="played_horizontal";
	}
	...
	//Call checkSquare recursively to check if a square has been made to count a point.
...
function verticalMove(pos,user) {
    ...
    //CPU played and made a square or no
	if (user == "c") {
		var a = document.getElementById(row+','+col);
		a.className="played_vertical";
	}
